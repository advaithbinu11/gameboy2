...
function preload(){
  ...
  bobWalkLeftImage = loadAnimation("alien1 copy.png","alien2 copy.png")
...
function setup() {
  ...
  bob.scale=0.8
  bob.addAnimation("bob", bobWalkImage);
  bob.x=58
  bob.y=80
...
function draw() {
  ...
  if(gameState==="play"){
    ...
    var sword2=createSprite(100,148);
    ...
    if (keyDown(LEFT_ARROW)&&downnum===0){
      bob.x=bob.x-5;
      bob.addAnimation(bobWalkLeftImage)
    }
    if(keyDown(RIGHT_ARROW)&&downnum===0){
      bob.x=bob.x+5;
      bob.addAnimation(bobWalkImage);
    }
    if(guardnum===0){
      guard.velocityX=2;
